...
export function RouteMap({
  ...
  // Initialize static background and legend only once
  useEffect(() => {
    ...
    // Create static background group
    ...
    // Create dynamic trails group (will be updated separately)
    svg.append('g').attr('class', 'trails-layer');
  ...
  // Update trails on timeline changes
  useEffect(() => {
    if (!svgRef.current || !backgroundInitializedRef.current) return;

    const svg = d3.select(svgRef.current);
    const trailsLayer = svg.select('.trails-layer');

    // Clear only the trails layer
    trailsLayer.selectAll('*').remove();

    const { padding } = MAP_CONFIG.visual;
    ...
    const xScale = d3
      ...
      .range([padding.left, MAP_CONFIG.visual.width - padding.right]);

    const yScale = d3
      .scaleLinear()
      .domain([bounds.maxLatitude, bounds.minLatitude])
      .range([padding.top, MAP_CONFIG.visual.height - padding.bottom]);
    ...
    const drawTrail = (
      ...
      const trailGroup = trailsLayer.append('g')
